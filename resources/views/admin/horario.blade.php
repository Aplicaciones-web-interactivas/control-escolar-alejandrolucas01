...
<body class="bg-gradient-to-br from-slate-50 via-white to-indigo-50 min-h-screen text-slate-800 font-sans antialiased">
    ...
    <main class="max-w-[85%] mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        @if (session('success'))
            <div
                class="mb-8 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center gap-3 text-emerald-800 shadow-sm animate-in fade-in slide-in-from-top-4">
                        ...
                        @forelse($horarios as $horario)
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-semibold text-slate-900">{{ $horario->materia->nombre ?? 'N/A' }}
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                    <div class="flex flex-wrap gap-1">
                                        @foreach (explode(', ', $horario->dia) as $d)
                                            <span
                                                class="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-md text-xs font-medium border border-indigo-100">{{ $d }}</span>
                                        @endforeach
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-mono">
                                    {{ date('H:i', strtotime($horario->hora_inicio)) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-mono">
                                    {{ date('H:i', strtotime($horario->hora_fin)) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm font-semibold text-slate-900">{{ $horario->usuario->name ?? 'No asignado' }}</span>
                                        <span
                                            class="text-[10px] text-slate-400 tracking-tighter">{{ $horario->usuario->clave_institucional ?? '' }}</span>
                                    </div>
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-2">
                                        <a href="{{ route('create.grupo', $horario->id) }}"
                                        ...
                                        <a href="{{ route('modificar.horario', $horario->id) }}"
                                        ...
                                        <form action="{{ route('delete.horario', $horario->id) }}" method="POST"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="mt-1 text-slate-500 max-w-sm mx-auto">Comienza asignando horarios a
                                            tus materias para verlos reflejados aquí.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
